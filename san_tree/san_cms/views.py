...
def AdminComplaintDashboard(request):
    ...
    complaints = Complaint.objects.filter(
        Q(department=user.department) | (Q(assigned_to=user) | Q(created_by=user))
    )
...
def UserComplaintDashboard(request):
    ...
    try:
        ...
        raise PermissionDenied("User profile not found")
    total_comp = (
        Complaint.objects.filter(created_by=user, assigned_to=user).all().count()
    )
    ...
    if start_date and end_date:
        try:
            ...
            complaints_created = complaints_created.filter(
                created_at__range=(start, end)
            )
            complaints_assigned = complaints_assigned.filter(
                created_at__range=(start, end)
            )
        except ValueError:
            pass
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if not complaint_type:
                ...
                if not types.exists():
                    types = ComplaintType.objects.filter(
                        name__iexact="Others", department__isnull=True
                    )
...
def AdminUpdateComplaintStatus(request, id):
    ...
    if (
        ...
        and complaint.complaint.department != user.department
    ):
...
def ComplainHistory(request):
    ...
    if selected_department_id:
        ...
        all_complaints = ComplaintHistory.objects.all().order_by(
            "-complaint__created_at"
        )
    ...
    context = {
        ...
        "selected_department": (
            int(selected_department_id) if selected_department_id else None
        ),
    }
...
def ReassignDepartmentView(request, complaint_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            assign_user = CustomUsers.objects.filter(
...
def AssignedTaskDetails(request, id):
    ...
    context = {"task": complaint, "remarks": remark}
    view_name = request.resolver_match.view_name
