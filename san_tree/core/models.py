...
class AnonymousServiceGenerate(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if (
            is_new
            and self.service_number.startswith("ASG")
            and self.service_number == f"GSN{AnonymousServiceGenerate.objects.count()}"
        ):
            self.service_number = f"ASG{self.id}"
            AnonymousServiceGenerate.objects.filter(pk=self.pk).update(
