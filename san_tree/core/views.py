...
def TrackView(request):
    ...
    if not query:
        return JsonResponse(
            {"found": False, "error": "Please enter a tracking number."}
        )
    ...
    return JsonResponse(
        {"found": False, "error": "No match found for this tracking number."}
    )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                register_dept.save()
                messages.success(
                    request, f'Department "{register_dept.name}" added successfully!'
                )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            exists = ComplaintType.objects.filter(
                name__iexact=comp_type.name, department=comp_type.department
            ).exists()
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            exists = TasksTypes.objects.filter(
                ...
                department__name__iexact=task_type.department,
            ).exists()
            if exists:
...
def AddServiceTypeView(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            exists = ServiceTypes.objects.filter(
                name__iexact=service_type.name
            ).exists()
            if exists:
            ...
            for staff in provider_staff:
                if staff.status == "vacant" or staff.status == "Vacant":
                    schedule = ShiftSchedule.objects.filter(
                        shift_staffs=staff, start_time__lte=now, end_time__gte=now
                    ).first()
                    if schedule:
                        ...
                        messages.error(
                            request, f"No shift schedule found for staff {staff}"
                        )
