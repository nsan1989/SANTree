...
class AssetModel(models.Model):
    ...
    name = models.CharField(max_length=100)
    category = models.ForeignKey(
        AssetCategoryModel, on_delete=models.CASCADE, related_name="asset_category"
    )
    manufacturer = models.CharField(max_length=100)
    ...
    asset_component = models.ManyToManyField(
        ComponentModel, blank=True, related_name="asset_components"
    )
    asset_consumable = models.ManyToManyField(
        ConsumableModel, blank=True, related_name="asset_consumables"
    )
    asset_accessory = models.ManyToManyField(
        AccessoryModel, blank=True, related_name="asset_accessories"
    )
    ...
    def save(self, *args, **kwargs):
        if not self.asset_tag:
            with transaction.atomic():
                last_asset = (
                    AssetModel.objects.select_for_update()
                    .order_by("-created_at")
                    .first()
                )
...
class AssetRequest(models.Model):
    asset = models.ForeignKey(
        AssetModel, on_delete=models.CASCADE, related_name="requests"
    )
